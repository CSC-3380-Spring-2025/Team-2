...
export default function AddMenu() { // if false dont show page
    let [name, setName] = React.useState('');
    let [price, setPrice] = React.useState('');
    let [type, setType] = React.useState('');
    let [img, setImg] = React.useState('');
    let [sale, setSale] = React.useState(false);
    ...
    async function onSubmit() {
        await setDoc(doc(db, "menu", name), { // auto generates doc id
            img: img,
            itemtype: type,
            name: name,
        ...
        <SafeAreaView style={{
            ...
            <View id='enterInfo'>
                ...
                <View style={{
                    ...
                    <Text>{'Sale?'}</Text>
                    <TouchableOpacity
                        style={{ marginLeft: 10, flex: 1, minHeight: 20, minWidth: 20, maxHeight: 20, maxWidth: 20, borderWidth: 3, backgroundColor: sale ? 'transparent' : '#1F645F' }}
                        onPress={() => { checkBox() }}>
